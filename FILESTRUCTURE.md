...
* C++ source files:
  * `include/kdtrace/kdtrace.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/kdtrace.cpp` is the main file that implements this library.
  * `app/kdtrace_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/kdtrace_t.cpp` contains the unit tests for the library.
  ...
  * The `python/kdtrace` directory contains a Python
    package for the project. It contains a compiled Python module `_kdtrace`
    that CMake generates from the Pybind11 source file `python/kdtrace/_kdtrace.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
